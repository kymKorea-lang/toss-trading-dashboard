...
export async function loadStocks() {
  ...
  try {
    ...
    ALL_STOCKS = [...krx, ...US_STOCKS];
    _loaded    = true;
    console.log(`✅ 종목 로드 완료: ${ALL_STOCKS.length}개`);
  } catch (e) {
...
function parseDirect(input) {
  const t = input.trim();
  if (/^\d{6}$/.test(t)) {
    const found = ALL_STOCKS.find(s => s.sym === t);
    return found || { name: t, abbr: t, sym: t, market: 'KR' };
  }
  if (/^[A-Za-z]{1,5}(\.[A-Z])?$/.test(t)) {
    const up    = t.toUpperCase();
    const found = ALL_STOCKS.find(s => s.sym === up);
